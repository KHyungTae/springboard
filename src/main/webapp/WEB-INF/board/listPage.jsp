	...
	function setPageList(page) {
		...
		$.ajax({
			...
			success: function(res) {
				...
				const list = res.resultList;//v
				const pageInfo = res.pageInfo;//v
				
				totalPages = pageInfo.totalPages; //총 페이지 수
				currentPage = page; //현재 페이지 초기화
				const totalItems = pageInfo.totalItems; //총 게시글 수//v
				let htmls = '';//v
				...
				$.each(list, function(idx, board) {
					...
					if(board.parent_id == 0) {
						htmls += '<a href="#" role="button" class="view_page" data_id="'+board.board_id+'">'+board.title+'</a>';
					} else {
						htmls += '<a href="#" role="button" class="view_page" data_id="'+board.board_id+'" style="margin-left: '+board.depth * 20+'px;">└ '+board.title+'</a>';
					}
					htmls += '</td>';
